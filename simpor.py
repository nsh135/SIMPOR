...
def uncertainty_sampling( X_train, threshold,model):
    ...
    based on high entropy samples 
    Prameters:
    X_train: entire trianing set
    ...
    #take high entropy portion of predictions (threshold = 0.1 will take 10% of highest entropy samples)
    selected_indices = np.argsort(predictions_entropy)[-int(size*threshold):]
    
    #plot histogram for debug purpose
    global onetime_plot 
    if onetime_plot:
        fig = px.histogram(x=predictions_entropy, )
        fig.write_image(file='LOG/Entropy_histogram.png', format='png'); onetime_plot=False
...
def gd( X, y, minor_cls, major_cls, h, radius, x,  max_iters=100, lr=0.01, tolerance=0.0000001,kde_ratio=True, verbose=0, CUDA= False ):
    ...
    *note: if CUDA: x, radius are arrays instead of scalars 
    ...
    def Ratio_Kde(x):
        """
        This is the ratio posterio function we want to minimize
        -----
...
def max_FracPosterior_balancing(X, y, k=1 , h = 0.3, AL_classifier=None, informative_threshold = 0.4,  n_threads=1, CUDA= False):
    ...
    print("Local balancing for shape", X.shape, y.shape)
